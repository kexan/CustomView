...
import com.kexan.customview.utils.AndroidUtils
...
class StatsView @JvmOverloads constructor(
    ...
    private val firstColorPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for ((index, datum) in data.withIndex()) {
            ...
            if (index == 0) {
                firstColorPaint.color = paint.color
            }
            canvas.drawArc(oval, startFrom, angle, false, paint)
            startFrom += angle
        }

        canvas.drawPoint(center.x, center.y - radius, firstColorPaint)
